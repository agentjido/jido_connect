defmodule Jido.Connect.Google.SearchConsole.Handlers.Actions.ResourceHelpers do
  @moduledoc false

  alias Jido.Connect.{Data, Error}
  alias Jido.Connect.Google.SearchConsole.Client

  def fetch_client(%{google_search_console_client: client}) when is_atom(client),
    do: {:ok, client}

  def fetch_client(_credentials), do: {:ok, Client.Sites}
  ...
  defp invalid_field(field, reason) do
    validation_error("Google Search Console #{field} must be a non-empty string",
      reason: reason,
      details: %{field: field}
    )
  end
