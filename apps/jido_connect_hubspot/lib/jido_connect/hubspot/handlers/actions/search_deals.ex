defmodule Jido.Connect.HubSpot.Handlers.Actions.SearchDeals do
  @moduledoc false
  ...
  def run(input, %{credentials: credentials}) do
    with {:ok, client} <- ResourceHelpers.fetch_client(credentials),
         ...
         {:ok, result} <- client.search_deals(normalize_input(input), token) do
  ...
  defp build_result(result) do
    base = %{
      deals: Enum.map(Map.get(result, :items, []), &ResourceHelpers.public_map/1)
    }
