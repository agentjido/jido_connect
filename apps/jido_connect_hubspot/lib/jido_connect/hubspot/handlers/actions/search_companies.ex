defmodule Jido.Connect.HubSpot.Handlers.Actions.SearchCompanies do
  @moduledoc false
  ...
  def run(input, %{credentials: credentials}) do
    with {:ok, client} <- ResourceHelpers.fetch_client(credentials),
         ...
         {:ok, result} <- client.search_companies(normalize_input(input), token) do
  ...
  defp build_result(result) do
    base = %{
      companies: Enum.map(Map.get(result, :items, []), &ResourceHelpers.public_map/1)
    }
