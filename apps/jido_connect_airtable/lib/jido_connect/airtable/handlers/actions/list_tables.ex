defmodule Jido.Connect.Airtable.Handlers.Actions.ListTables do
  @moduledoc false
  ...
  def run(input, %{credentials: credentials}) do
    with {:ok, client} <- ResourceHelpers.fetch_client(credentials),
         ...
         {:ok, result} <- client.list_tables(input, token) do
  ...
  defp build_result(result) do
    base = %{
      tables: Enum.map(Map.get(result, :tables, []), &ResourceHelpers.public_map/1)
    }
