defmodule Jido.Connect.Airtable.Client.TransportTest do
  use ExUnit.Case, async: true

  alias Jido.Connect.Airtable.Client.Transport

  test "base_url returns default Airtable API URL" do
    assert Transport.base_url() == "https://api.airtable.com"
  end
  ...
  test "api_request builds a bearer request" do
    ...
    assert request.options.base_url == "https://api.airtable.com"
  ...
  test "invalid_success_response returns provider error" do
    assert {:error,
            %Jido.Connect.Error.ProviderError{provider: :airtable, reason: :invalid_response}} =
