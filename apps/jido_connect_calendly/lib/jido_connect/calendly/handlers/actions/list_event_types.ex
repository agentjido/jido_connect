defmodule Jido.Connect.Calendly.Handlers.Actions.ListEventTypes do
  @moduledoc false
  ...
  def run(input, %{credentials: credentials}) do
    with {:ok, client} <- ResourceHelpers.fetch_client(credentials),
         ...
         {:ok, result} <- client.list_event_types(normalize_input(input), token) do
  ...
  defp build_result(result) do
    base = %{
      event_types: Enum.map(Map.get(result, :items, []), &ResourceHelpers.public_map/1)
    }
