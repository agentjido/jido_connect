defmodule Jido.Connect.MicrosoftCalendar.Handlers.Actions.TentativelyAcceptEvent do
  @moduledoc false

  alias Jido.Connect.Microsoft.Transport

  @doc """
  Tentatively accepts a Microsoft Calendar event invitation.

  POST /me/events/{event_id}/tentativelyAccept with an optional comment.
    ...
    case Map.get(input, :event_id) do
      ...
      event_id ->
        ...
        case Transport.request(request, :post,
          ...
          {:ok, %{status: 202}} ->
            {:ok, %{tentatively_accepted: true, event_id: event_id}}
          ...
          {:ok, response} ->
            Transport.handle_error_response({:ok, response},
              message: "Failed to tentatively accept Microsoft calendar event"
            )
