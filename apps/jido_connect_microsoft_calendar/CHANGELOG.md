...
- Initial scaffold: integration DSL, auth profile reuse from the Microsoft
  ...
  tests.
- Action catalog shell listing planned calendar read, write, and delete
  actions. Handlers return `{:error, :not_implemented}` until full calendar and
  event actions are added.
