defmodule Jido.Connect.Linear.OAuthTest do
  @moduledoc """
  Offline tests for Linear OAuth2 token exchange.
  ...
  describe "exchange_code/2" do
    ...
    test "returns OAuth error response" do
      ...
      assert {:error, %Error.ProviderError{provider: :linear, reason: "invalid_grant"} = error} =
    ...
    test "normalizes malformed successful token responses" do
      ...
      assert {:error, %Error.ProviderError{provider: :linear, reason: :invalid_response} = error} =
               OAuth.exchange_code("code",
                 client_id: "client",
                 client_secret: "secret",
                 redirect_uri: "https://demo.test/callback",
    ...
    test "normalizes transport errors" do
      ...
      assert {:error, %Error.ProviderError{provider: :linear, reason: :request_error} = error} =
               OAuth.exchange_code("code",
                 client_id: "client",
                 client_secret: "secret",
                 redirect_uri: "https://demo.test/callback",
