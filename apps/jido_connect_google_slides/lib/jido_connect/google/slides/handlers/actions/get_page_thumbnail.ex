defmodule Jido.Connect.Google.Slides.Handlers.Actions.GetPageThumbnail do
  @moduledoc false
  ...
  def run(input, %{credentials: credentials}) do
    with {:ok, client} <- fetch_client(credentials),
         {:ok, thumbnail} <-
           client.get_page_thumbnail(input, Map.get(credentials, :access_token)) do
      {:ok, %{thumbnail: public_map(thumbnail)}}
    end
  end
