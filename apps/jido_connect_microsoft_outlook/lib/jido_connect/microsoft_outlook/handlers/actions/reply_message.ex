defmodule Jido.Connect.MicrosoftOutlook.Handlers.Actions.ReplyMessage do
  @moduledoc false
  ...
  POST /me/messages/{message_id}/reply with a JSON body containing the comment.
        ...
        case Transport.request(request, :post,
          ...
          {:ok, response} ->
            Transport.handle_error_response({:ok, response},
              message: "Failed to reply to Outlook message"
            )

          {:error, _reason} = error ->
            Transport.handle_error_response(error, message: "Failed to reply to Outlook message")
        end
    end
  end
