defmodule Jido.Connect.Salesforce.Handlers.Actions.UpdateRecord do
  @moduledoc false
  ...
  def run(input, %{credentials: credentials}) do
    with {:ok, client} <- ResourceHelpers.fetch_client(credentials),
         ...
         {:ok, _result} <- client.update_record(input, credentials) do
      {:ok, %{record_id: input.record_id, success: true}}
    end
  end
