defmodule Jido.Connect.Salesforce.Handlers.Actions.UpdateContact do
  @moduledoc false
  ...
  def run(input, %{credentials: credentials}) do
    with {:ok, client} <- ResourceHelpers.fetch_client(credentials),
         ...
         {:ok, _result} <- client.update_contact(input, credentials) do
      {:ok, %{contact_id: input.contact_id, success: true}}
    end
  end
