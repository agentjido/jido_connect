defmodule Jido.Connect.Google.Forms.Handlers.Actions.CreateWatch do
  @moduledoc false
  ...
  def run(input, %{credentials: credentials}) do
    with :ok <- validate_input(input),
         ...
         {:ok, watch} <-
           client.create_watch(input, Map.get(credentials, :access_token)) do
  ...
  defp validate_input(input) do
    with :ok <- require_present(input, :form_id),
         :ok <- validate_event_type(input) do
      :ok
    end
  end

  defp validate_event_type(input) do
    case Map.get(input, :event_type) do
      event_type when event_type in @event_types ->
        :ok

      other ->
        {:error,
         Error.validation("Google Forms watch event_type is invalid",
           reason: :invalid_forms_watch,
           details: %{field: :event_type, value: other, allowed: @event_types}
         )}
    end
  end
