defmodule Jido.Connect.Google.Forms.Handlers.Actions.GetResponse do
  @moduledoc false
  ...
  def run(input, %{credentials: credentials}) do
    with {:ok, client} <- fetch_client(credentials),
         {:ok, response} <-
           client.get_response(input, Map.get(credentials, :access_token)) do
      {:ok, %{response: public_map(response)}}
    end
  end
