defmodule Jido.Connect.Intercom.Client.Transport do
  @moduledoc """
  Intercom REST transport boundary.

  Provides bearer-based request building for Intercom access tokens.
  All requests target the Intercom REST API at `https://api.intercom.io`.
  """

  alias Jido.Connect.{Error, Provider.Transport}

  @doc "Builds an Intercom API bearer request."
  @spec request(String.t(), keyword()) :: Req.Request.t()
  def request(access_token, opts \\ []) when is_binary(access_token) and is_list(opts) do
    Transport.bearer_request(
      Keyword.get(opts, :base_url, base_url()),
      access_token,
      headers: [
        {"accept", "application/json"},
        {"intercom-version", "2.11"}
      ],
      req_options:
        Application.get_env(:jido_connect_intercom, :intercom_req_options, [])
        |> Keyword.merge(Keyword.get(opts, :req_options, []))
    )
  end

  @doc "Returns the configured Intercom API base URL."
  @spec base_url() :: String.t()
  def base_url do
    Application.get_env(
      :jido_connect_intercom,
      :intercom_api_base_url,
      "https://api.intercom.io"
    )
  end

  @doc "Normalizes an Intercom provider error response."
  @spec handle_error_response(term(), keyword()) :: {:error, Error.ProviderError.t()}
  def handle_error_response(response, opts \\ [])

  def handle_error_response({:ok, %{status: status, body: body}} = response, opts)
      when is_integer(status) and is_map(body) do
    message = Keyword.get(opts, :message, "Intercom API request failed")

    Transport.provider_error(
      response,
      Keyword.merge(opts,
        provider: :intercom,
        message: message,
        reason: Keyword.get(opts, :reason, reason_from_status(status)),
        detail_message: intercom_error_message(body)
      )
    )
  end

  def handle_error_response(response, opts) do
    message = Keyword.get(opts, :message, "Intercom API request failed")
    Transport.provider_error(response, Keyword.merge(opts, provider: :intercom, message: message))
  end

  @doc "Returns a sanitized provider error for malformed success payloads."
  @spec invalid_success_response(String.t(), term()) :: {:error, Error.ProviderError.t()}
  def invalid_success_response(message, body) do
    {:error,
     Error.provider(message,
       provider: :intercom,
       reason: :invalid_response,
       details: %{body: body}
     )}
  end

  defp reason_from_status(401), do: :unauthorized
  defp reason_from_status(403), do: :forbidden
  defp reason_from_status(429), do: :rate_limited
  defp reason_from_status(status) when status in 500..599, do: :server_error
  defp reason_from_status(_status), do: :http_error

  defp intercom_error_message(%{"errors" => [%{"message" => msg} | _]}), do: msg
  defp intercom_error_message(%{"error" => error}) when is_binary(error), do: error
  defp intercom_error_message(%{"message" => message}) when is_binary(message), do: message
  defp intercom_error_message(%{"title" => title}) when is_binary(title), do: title
  defp intercom_error_message(_body), do: "Intercom API request failed"
end
