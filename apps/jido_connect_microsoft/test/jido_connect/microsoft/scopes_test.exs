defmodule Jido.Connect.Microsoft.ScopesTest do
  ...
  test "exposes initial product scope catalog" do
    ...
    assert "Sites.Read.All" in Scopes.product(:sharepoint)
    assert "Sites.Selected" in Scopes.product(:sharepoint)
    assert "Lists.SelectedOperations.Selected" in Scopes.product(:sharepoint)
    assert "Files.SelectedOperations.Selected" in Scopes.product(:sharepoint)
