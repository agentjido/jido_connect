defmodule Jido.Connect.Zendesk.OAuthTest do
  @moduledoc """
  Offline tests for Zendesk OAuth2 token exchange.
  ...
  alias Jido.Connect.Zendesk.OAuth
  ...
  setup do
    Application.put_env(
      ...
      :zendesk_oauth_req_options,
      plug: {Req.Test, __MODULE__}
    )

    on_exit(fn ->
      Application.delete_env(:jido_connect_zendesk, :zendesk_oauth_req_options)
    end)
  end

  describe "authorize_url/1" do
    test "builds authorize URL with Zendesk parameters" do
      url =
        OAuth.authorize_url(
          client_id: "client",
          redirect_uri: "https://demo.test/integrations/zendesk/oauth/callback",
          scope: ["read", "tickets:read"],
          state: "state",
          subdomain: "mycompany"
        )
      ...
      assert uri.host == "mycompany.zendesk.com"
      assert uri.path == "/oauth/authorizations/new"
      assert params["client_id"] == "client"
      assert params["redirect_uri"] == "https://demo.test/integrations/zendesk/oauth/callback"
      assert params["scope"] == "read tickets:read"
  ...
  describe "exchange_code/2" do
    test "exchanges code for token with correct request body" do
      ...
      assert token.scope == ["read", "tickets:read"]
    ...
    test "returns OAuth error response" do
      ...
      assert {:error, %Error.ProviderError{provider: :zendesk, reason: "invalid_grant"} = error} =
    ...
    test "normalizes malformed successful token responses" do
      ...
      assert {:error, %Error.ProviderError{provider: :zendesk, reason: :invalid_response} = error} =
               OAuth.exchange_code("code",
                 client_id: "client",
                 client_secret: "secret",
                 redirect_uri: "https://demo.test/callback",
    ...
    test "normalizes transport errors" do
      ...
      assert {:error, %Error.ProviderError{provider: :zendesk, reason: :request_error} = error} =
               OAuth.exchange_code("code",
                 client_id: "client",
                 client_secret: "secret",
                 redirect_uri: "https://demo.test/callback",
