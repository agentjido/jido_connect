defmodule Jido.Connect.PostHog.Handlers.Actions.ListEvents do
  @moduledoc false
  ...
  def run(input, %{credentials: credentials}) do
    with {:ok, client} <- fetch_client(credentials),
         ...
         {:ok, %Req.Response{status: 200, body: body}} <-
           client.list_events(token, build_opts(input)) do
      {:ok, normalize_events(body)}
    else
  ...
  defp build_opts(input) do
    ...
    |> maybe_opt(:event, Map.get(input, :event))
    |> maybe_opt(:distinct_id, Map.get(input, :distinct_id))
  end

  defp maybe_opt(opts, _key, nil), do: opts
  defp maybe_opt(opts, key, value), do: Keyword.put(opts, key, value)

  defp normalize_events(%{"results" => results} = body) do
    %{
      events: results,
  ...
  defp normalize_events(body), do: body
end
